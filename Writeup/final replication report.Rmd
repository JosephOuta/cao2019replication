...
For the key finding, finding 2 (Bayesian judgments regardless of person X evaluations), the Cohen's f effect size was calculated to be 0.48, and the sample size that would achieve 80% power was calculated to be 36. 


### Planned Sample

The sample sizes 36 for finding 2 and 37 for finding 1 make it feasible to attempt to replicate both findings simultaneously with a sample size of 37. To account for possible exclusions, a sample of 38 American Mturk workers will be ran. 
...
* In addition to the two criteria used in the original study, this study will incorporate an additional attention check at the end of the survey that asks participants to indicate whether they were distracted, had trouble paying attention, or whether something else negatively affected their participation. Participants will be excluded who agree to any one of the above statements.
...
* A second difference is the addition of a self-reported attention check at the end of the study. This will help filter out participants whose responses are negatively affected by distractions.   
...
### Person X evaluations - Finding 1
...
With an alpha level of 0.05, the results of the ANOVA from replicating finding 2 are as follows:   
F(1, 32) = 7.4, p = 0.01, η2 = 0.18, 95% CI = [0.01, 0.39]   
Cohen’s f: 0.59

In contrast, the original results were as follows:   
...
anova(lm(formula = posterior_percent ~ composite.avg.shifted * target, 
         data = dat)) # There was a statistically significant main effect of target on posterior judgments F(1, 32) = 7.4, p = 0.01, etasquared = 0.18, 95% CI = [0.01, 0.39] (Finding 2)
...
There are two statistical tests involved in this replication, the first is the primary replication target, the ANOVA with linear model fit; and the secondary target is the two-tailed t-test. The ANOVA assesses the extent to which the target (man vs woman) and evaluations of person X (Likert scale from 1 to 7) are related to posterior judgments whereas the t-test assesses the extent to which the evaluations of person X are positive or negative.   

We were able to successfully replicate the secondary target, finding the judgments to be negative with a composite average p-value of 0.004. However we were only able to partially replicate the primary target, with a successful replication of the main effect of the target on posterior judgments having a p-value of 0.01 and Cohen's f of 0.59, but a failed replication of the trend showing an increase in differences between male and female targets as person X evaluations became more positive. Instead, an opposite trend was seen, one in which the differences decreased as evaluations became more positive. 

 
### Commentary

In order to understand the partial replication of finding 2, a follow-up exploratory analysis was conducted. This would provide some insight into the failed attempt to replicate figure 4. The figure was re-plotted, this time incorporating individual data points to investigate the point distribution underlying the fitted regression lines. 
